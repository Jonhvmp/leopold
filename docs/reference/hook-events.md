# Hook Events — Live Probe of Both Harnesses

**The question:** which lifecycle hook events actually fire from a *headless* session on the installed binaries, with what payload, and which reply does each harness honor? Leopold's rules become hooks only on events that were captured, never on a docs page or a `strings` dump. This page is generated by `scripts/probe-hook-events.sh` from the verbatim payloads it recorded; rerun it on both binaries and the page rewrites itself.

**The answer in one line:** 26 of the 33 events Claude Code documents fired on `2.1.259 (Claude Code)`; 12 of the 12 events Codex CLI documents fired on `codex-cli 0.152.1`. Every event that did not fire is recorded below with the trigger tried.
...
| Claude Code model alias for the probe | `haiku` |
| Codex model (from the captured payloads) | `gpt-5.6-sol` |
| jq | `jq-1.7.1-apple` |
| Python (stubs, renderer) | `Python 3.14.6` |
| bash (hooks, driver) | `3.2.57(1)-release` |
| OS | `Darwin 27.0.0 arm64` |
| Leopold | 0.22.0 (`scripts/probe-hook-events.sh`, `scripts/probe/dump-hook.sh`) |
| Probed at (UTC) | 2026-09-03T01:36:54Z |

## Method (hermetic)

- A throwaway project under `mktemp -d` per harness (`/tmp/leopold-hookprobe.gjQnXe`): `git init`-ed, with a `CLAUDE.md` / `AGENTS.md`, a `.leopold/PLAN.md` and a root `PLAN.md` (the `FileChanged` control), a 120 KB file to compact against, a project command (`.claude/commands/probecmd.md`) and a Codex role file (`.codex/agents/probe-reviewer.toml`).
- `scripts/probe/dump-hook.sh <event>` wired on **every** event of each harness's documented matrix through the shared writer in `extensions/lib/harness.sh` — `leo_wire_hooks_json` into the project's `.claude/settings.json`, `leo_wire_hooks_toml` into a temp `CODEX_HOME/config.toml`; nothing pastes JSON or TOML by hand. The hook appends its stdin verbatim to `<out>/<harness>/<event>.jsonl` — the event name is its argument, so a payload without `hook_event_name` still files under the right name — and answers per reply mode (observe / allow / deny / exit 2 / `systemMessage` / `additionalContext` / elicitation accept / worktree).
- **Claude Code:** `claude -p --setting-sources project --output-format stream-json --include-hook-events` in a clean environment. The real config dir is only *read* (macOS keeps the login in the Keychain — exactly how [SDK Worker Hooks](sdk-worker-hooks.md) run 2 authenticated); a temp `CLAUDE_CONFIG_DIR` is used for one pass only, the unauthenticated *hooks-fire-anyway* run.
- **Codex CLI:** `codex exec --json --skip-git-repo-check --dangerously-bypass-hook-trust` with stdin closed, a temp `CODEX_HOME` holding a copy of `auth.json` that is deleted when the probe ends.
- One trigger per event: a passing and a failing Bash command; the session's own edit of `.leopold/PLAN.md` and an append from *another process*; an `Agent` / `spawn_agent` spawn; a TaskCreate + TaskUpdate completion with exit-2 blocking; forced compaction (`CLAUDE_CODE_AUTO_COMPACT_WINDOW` / `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`, `/compact` on a resumed session; Codex `-c model_auto_compact_token_limit`); an API failure through `ANTHROPIC_BASE_URL` and a Codex `model_providers.<id>.base_url` pointed at a Python-stdlib stub answering 429, 529, 500 and 401; a permission request for a non-allowlisted tool with allow and deny replies; a `settings.json` edit mid-session; an MCP elicitation from a stdio stub; `--worktree`; `--init` / `--maintenance`; `/model`; a `register_repo_root` control request; SIGINT to `codex exec`; a Codex role file exercised by `spawn_agent` and by `codex exec` with `--strict-config`.
- **Hermeticity is asserted, not assumed:** the entry names of the real `~/.claude` and `~/.codex` are fingerprinted before and after (the check `scripts/test-harness-lib.sh` runs) and the probe fails if either changed. This run: `~/.claude` unchanged, `~/.codex` CHANGED.
- Payloads below are quoted **verbatim**; only string values longer than 480 characters are cut with an explicit `…[+N chars]` marker. The raw files under `<out>/` hold every byte.

## Matrix — Claude Code (2.1.259 (Claude Code))

| Event | Fired | Runs | `hook_event_name` | `session_id` | Replies exercised |
| --- | --- | --- | --- | --- | --- |
| `SessionStart` | yes ×40 | add-dir, additional-context, command, command-exit2, compact-auto, compact-manual, compact-manual-seed, config-change, config-change-exit2, fail-401, fail-429, fail-500, fail-529, filechanged-cwd, filechanged-nested, model-switch, model-switch-exit2, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, prompt-exit2, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, unauth, worktree-agent | present | present | `additionalContext`, `systemMessage` |
| `Setup` | yes ×2 | setup-init, setup-maintenance | present | present | — |
| `UserPromptSubmit` | yes ×33 | add-dir, additional-context, command, compact-auto, compact-manual-seed, config-change, config-change-exit2, fail-401, fail-429, fail-500, fail-529, filechanged-cwd, filechanged-nested, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, prompt-exit2, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, unauth, worktree-agent | present | present | `additionalContext`, `exit2`, `systemMessage` |
| `UserPromptExpansion` | yes ×2 | command, command-exit2 | present | present | `exit2` |
| `PreToolUse` | yes ×48 | additional-context, compact-auto, compact-manual-seed, config-change, config-change-exit2, filechanged-cwd, filechanged-nested, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, subagent, subagent-exit2, system-message, tasks, tasks-exit2, tools, worktree-agent | present | present | `additionalContext`, `permissionDecision:deny`, `systemMessage` |
| `PermissionRequest` | yes ×2 | perm-allow, perm-deny | present | present | `decision:allow`, `decision:allow (host prompts)`, `decision:deny` |
| `PermissionDenied` | **not fired** | — | — | — | — |
| `PostToolUse` | yes ×43 | additional-context, compact-auto, compact-manual-seed, config-change, config-change-exit2, filechanged-cwd, filechanged-nested, perm-allow, posttool-block, subagent, subagent-exit2, system-message, tasks, tasks-exit2, tools, worktree-agent | present | present | `additionalContext`, `decision:block`, `systemMessage` |
| `PostToolUseFailure` | yes ×1 | tools | present | present | — |
| `PostToolBatch` | yes ×48 | additional-context, compact-auto, compact-manual-seed, config-change, config-change-exit2, filechanged-cwd, filechanged-nested, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, subagent, subagent-exit2, system-message, tasks, tasks-exit2, tools, worktree-agent | present | present | `additionalContext`, `systemMessage` |
| `Notification` | **not fired** | — | — | — | — |
| `MessageDisplay` | yes ×48 | add-dir, additional-context, command, compact-auto, compact-manual-seed, config-change, config-change-exit2, fail-401, fail-429, fail-500, fail-529, filechanged-cwd, filechanged-nested, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, unauth, worktree-agent | present | present | `systemMessage` |
| `SubagentStart` | yes ×3 | subagent, subagent-exit2, worktree-agent | present | present | — |
| `SubagentStop` | yes ×6 | compact-auto, compact-manual, subagent, subagent-exit2, worktree-agent | present | present | `exit2` |
| `TaskCreated` | yes ×2 | tasks, tasks-exit2 | present | present | — |
| `TaskCompleted` | yes ×2 | tasks, tasks-exit2 | present | present | `exit2` |
| `Stop` | yes ×29 | add-dir, additional-context, command, compact-auto, compact-manual-seed, config-change, config-change-exit2, filechanged-cwd, filechanged-nested, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, worktree-agent | present | present | `decision:block`, `exit2`, `systemMessage` |
| `StopFailure` | yes ×5 | fail-500 | present | present | — |
| `TeammateIdle` | **not fired** | — | — | — | — |
| `InstructionsLoaded` | yes ×38 | add-dir, additional-context, command, command-exit2, compact-auto, compact-manual, compact-manual-seed, config-change, config-change-exit2, fail-401, fail-429, fail-500, fail-529, filechanged-cwd, filechanged-nested, model-switch, model-switch-exit2, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, prompt-exit2, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, unauth, worktree-agent | present | present | `additionalContext`, `systemMessage` |
| `ConfigChange` | yes ×2 | config-change, config-change-exit2 | present | present | `exit2` |
| `CwdChanged` | yes ×2 | filechanged-cwd, tools | present | present | `systemMessage` |
| `DirectoryAdded` | **not fired** | — | — | — | — |
| `FileChanged` | yes ×6 | filechanged-nested, tools | present | present | — |
| `WorktreeCreate` | yes ×2 | worktree, worktree-agent | present | present | `worktree` |
| `WorktreeRemove` | **not fired** | — | — | — | — |
| `PreCompact` | yes ×2 | compact-auto, compact-manual | present | present | — |
| `PostCompact` | yes ×2 | compact-auto, compact-manual | present | present | — |
| `PreModelSwitch` | yes ×2 | model-switch, model-switch-exit2 | present | present | `exit2` |
| `PostModelSwitch` | yes ×1 | model-switch | present | present | — |
| `Elicitation` | **not fired** | — | — | — | `action:accept+content` |
| `ElicitationResult` | **not fired** | — | — | — | — |
| `SessionEnd` | yes ×34 | add-dir, additional-context, command, command-exit2, compact-auto, compact-manual, compact-manual-seed, config-change, config-change-exit2, fail-429, filechanged-cwd, filechanged-nested, model-switch, model-switch-exit2, perm-allow, perm-auto, perm-deny, perm-host, posttool-block, pretool-deny, prompt-exit2, setup-init, setup-maintenance, stop-block, stop-exit2, subagent, subagent-exit2, system-message, tasks, tasks-exit2, teams, tools, worktree-agent | present | present | `systemMessage` |

## Matrix — Codex CLI (codex-cli 0.152.1)

| Event | Fired | Runs | `hook_event_name` | `session_id` | Replies exercised |
| --- | --- | --- | --- | --- | --- |
| `SessionStart` | yes ×25 | additional-context, compact, exec-as-role-4, fail-401, fail-429, fail-500, fail-529, interrupt, perm-allow, perm-deny, perm-onrequest, posttool-block, pretool-deny, prompt-exit2, role-bogus, role-bogus-strict, stop-block, stop-exit2, strict-config, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `--strict-config with the probe's hook block`, `additionalContext`, `systemMessage` |
| `SessionEnd` | yes ×24 | additional-context, compact, exec-as-role-4, fail-401, fail-429, fail-500, fail-529, interrupt, perm-allow, perm-deny, perm-onrequest, posttool-block, pretool-deny, prompt-exit2, role-bogus, role-bogus-strict, stop-block, stop-exit2, strict-config, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `systemMessage` |
| `SubagentStart` | yes ×3 | subagent, subagent-exit2, subagent-role | present | present | `codex exec -c agent_role="probe-reviewer"`, `codex exec -c agent_type="probe-reviewer"`, `codex exec -c agents.probe-reviewer.config_file=".codex/agents/probe-reviewer.toml"`, `codex exec -c role="probe-reviewer"`, `codex exec -p <role file as profile>`, `role file`, `role file with an unknown key`, `role file with an unknown key (--strict-config)` |
| `SubagentStop` | yes ×4 | subagent, subagent-exit2, subagent-role | present | present | `exit2` |
| `PreToolUse` | yes ×29 | additional-context, compact, interrupt, perm-allow, perm-deny, perm-onrequest, posttool-block, pretool-deny, role-bogus, role-bogus-strict, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `additionalContext`, `permissionDecision:deny`, `systemMessage` |
| `PermissionRequest` | yes ×1 | perm-deny | present | present | `decision:allow`, `decision:allow (approval_policy=on-request)`, `decision:deny` |
| `PostToolUse` | yes ×24 | additional-context, compact, perm-allow, perm-deny, perm-onrequest, posttool-block, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `additionalContext`, `decision:block`, `systemMessage` |
| `PreCompact` | yes ×1 | compact | present | present | — |
| `PostCompact` | yes ×1 | compact | present | present | — |
| `UserPromptSubmit` | yes ×24 | additional-context, compact, exec-as-role-4, fail-401, fail-429, fail-500, fail-529, interrupt, perm-allow, perm-deny, perm-onrequest, posttool-block, pretool-deny, prompt-exit2, role-bogus, role-bogus-strict, stop-block, stop-exit2, strict-config, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `additionalContext`, `exit2`, `systemMessage` |
| `Stop` | yes ×20 | additional-context, compact, exec-as-role-4, perm-allow, perm-deny, perm-onrequest, posttool-block, pretool-deny, role-bogus, role-bogus-strict, stop-block, stop-exit2, strict-config, subagent, subagent-exit2, subagent-role, system-message, tools | present | present | `decision:block`, `exit2`, `systemMessage` |
| `Interrupt` | yes ×1 | interrupt | present | present | — |

## Events — Claude Code

### `SessionStart` — Claude Code

Fired 40 time(s) in run(s) `add-dir`, `additional-context`, `command`, `command-exit2`, `compact-auto`, `compact-manual`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `filechanged-cwd`, `filechanged-nested`, `model-switch`, `model-switch-exit2`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `unauth`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `context_tokens`, `cwd`, `estimated_cache_write_usd`, `hook_event_name`, `model`, `prompt_cache_likely_expired`, `prompt_id`, `seconds_since_last_response`, `session_id`, `source`, `transcript_path`.
...
Replies the harness honored:

- `systemMessage` → **honored** — marker PROBE_SYSMSG SessionStart appears 1 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX SessionStart': yes; the marker is in the transcript 3 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `Setup` — Claude Code

Fired 2 time(s) in run(s) `setup-init`, `setup-maintenance`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `session_id`, `transcript_path`, `trigger`.
...
Fired 33 time(s) in run(s) `add-dir`, `additional-context`, `command`, `compact-auto`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `filechanged-cwd`, `filechanged-nested`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `unauth`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `permission_mode`, `prompt`, `prompt_id`, `session_id`, `transcript_path`.
...
- `exit2` → **honored** — hook_response: outcome=error exit_code=2; assistant messages carrying the prompt's marker: 0; Stop fired: 0; final: UserPromptSubmit operation blocked by hook:  *(run prompt-exit2)*
- `systemMessage` → **honored** — marker PROBE_SYSMSG UserPromptSubmit appears 2 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX UserPromptSubmit': no; the marker is in the transcript 2 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `UserPromptExpansion` — Claude Code

Fired 2 time(s) in run(s) `command`, `command-exit2`. `hook_event_name`: present; `session_id`: present. Payload keys: `command_args`, `command_name`, `command_source`, `cwd`, `expansion_type`, `hook_event_name`, `permission_mode`, `prompt`, `prompt_id`, `session_id`, `transcript_path`.
...
- `permissionDecision:deny` → **honored** — PostToolUse fired 0 times after the deny (0 = the tool never ran); reason reached the model: yes *(run pretool-deny)*
- `systemMessage` → **honored** — marker PROBE_SYSMSG PreToolUse appears 2 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX PreToolUse': no; the marker is in the transcript 3 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `PermissionRequest` — Claude Code

Fired 2 time(s) in run(s) `perm-allow`, `perm-deny`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `permission_mode`, `permission_suggestions`, `prompt_id`, `session_id`, `tool_input`, `tool_name`, `transcript_path`.
...
- `decision:allow` → **honored** — the file the command creates exists: yes; PostToolUse fired: 1 *(run perm-allow)*
- `decision:deny` → **honored** — the file exists: no; message PROBE_DENY reached the model: yes; PermissionDenied fired: 0 *(run perm-deny)*
- `decision:allow (host prompts)` → **not-honored** — with --permission-prompts host (the default) the hook fired 0 times and the file exists: no — permission_denials in the result: 1 *(run perm-host)*

### `PermissionDenied` — Claude Code

**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run perm-auto: --permission-mode auto with a Bash touch (denied, permission_denials=1); also run perm-deny: a PermissionRequest hook answering deny

### `PostToolUse` — Claude Code

Fired 43 time(s) in run(s) `additional-context`, `compact-auto`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `filechanged-cwd`, `filechanged-nested`, `perm-allow`, `posttool-block`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `tools`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `agent_id`, `agent_type`, `cwd`, `duration_ms`, `hook_event_name`, `permission_mode`, `prompt_id`, `session_id`, `tool_input`, `tool_name`, `tool_response`, `tool_use_id`, `transcript_path`.
...
- `decision:block` → **honored** — hook_response: outcome=success exit_code=0; the reason PROBE_BLOCK reached the model/stream: yes *(run posttool-block)*
- `systemMessage` → **honored** — marker PROBE_SYSMSG PostToolUse appears 2 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX PostToolUse': no; the marker is in the transcript 3 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `PostToolUseFailure` — Claude Code

Fired 1 time(s) in run(s) `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `duration_ms`, `error`, `hook_event_name`, `is_interrupt`, `permission_mode`, `prompt_id`, `session_id`, `tool_input`, `tool_name`, `tool_use_id`, `transcript_path`.
...
- `systemMessage` → **honored** — marker PROBE_SYSMSG PostToolBatch appears 2 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX PostToolBatch': no; the marker is in the transcript 3 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `Notification` — Claude Code

**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run tools: a headless `claude -p` turn in a project with a CLAUDE.md that runs `true`, `false`, `grep -c zzz /dev/null`, an Edit of .leopold/PLAN.md and of the root PLAN.md, `sleep 6` (the driver appends to both files from another process meanwhile), `cat` and, last, `cd sub && pwd` · runs perm-allow / perm-deny / perm-host: a permission decision in a headless session (matcher permission_prompt)

### `MessageDisplay` — Claude Code

Fired 48 time(s) in run(s) `add-dir`, `additional-context`, `command`, `compact-auto`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `filechanged-cwd`, `filechanged-nested`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `unauth`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `delta`, `final`, `hook_event_name`, `index`, `message_id`, `prompt_id`, `session_id`, `transcript_path`, `turn_id`.
...
- `systemMessage` → **honored** — marker PROBE_SYSMSG MessageDisplay appears 1 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*

### `SubagentStart` — Claude Code

Fired 3 time(s) in run(s) `subagent`, `subagent-exit2`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `agent_id`, `agent_type`, `cwd`, `hook_event_name`, `prompt_id`, `session_id`, `transcript_path`.
...
Fired 6 time(s) in run(s) `compact-auto`, `compact-manual`, `subagent`, `subagent-exit2`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `agent_id`, `agent_transcript_path`, `agent_type`, `background_tasks`, `cwd`, `hook_event_name`, `last_assistant_message`, `permission_mode`, `prompt_id`, `session_crons`, `session_id`, `stop_hook_active`, `transcript_path`.
...
- `exit2` → **honored** — SubagentStop fired 2 times in the run (exit 2 once, then the subagent stopped again); stop_hook_active on the second: true; hook_response: outcome=success exit_code=0 *(run subagent-exit2)*

### `TaskCreated` — Claude Code

Fired 2 time(s) in run(s) `tasks`, `tasks-exit2`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `prompt_id`, `session_id`, `task_description`, `task_id`, `task_subject`, `transcript_path`.
...
- `exit2` → **honored** — hook_response: outcome=error exit_code=2; final reply: Task created, updated to completed, and here's the status from TaskList:  *(run tasks-exit2)*

### `Stop` — Claude Code

Fired 29 time(s) in run(s) `add-dir`, `additional-context`, `command`, `compact-auto`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `filechanged-cwd`, `filechanged-nested`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `background_tasks`, `cwd`, `hook_event_name`, `last_assistant_message`, `permission_mode`, `prompt_id`, `session_crons`, `session_id`, `stop_hook_active`, `transcript_path`.
...
- `exit2` → **honored** — Stop fired 2 times (exit 2 once; the model continued and stopped again); stop_hook_active on the second: true; hook_response: outcome=success exit_code=0 *(run stop-exit2)*
- `decision:block` → **honored** — Stop fired 2 times ({"decision":"block"} once); stop_hook_active on the second: true *(run stop-block)*
- `systemMessage` → **honored** — marker PROBE_SYSMSG Stop appears 2 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*

### `StopFailure` — Claude Code

Fired 5 time(s) in run(s) `fail-500`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `error`, `hook_event_name`, `last_assistant_message`, `prompt_id`, `session_id`, `transcript_path`.
...
**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run teams: CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 and a prompt asking for TeamCreate + one teammate going idle (reply: NO_TEAM_TOOL)

### `InstructionsLoaded` — Claude Code

Fired 38 time(s) in run(s) `add-dir`, `additional-context`, `command`, `command-exit2`, `compact-auto`, `compact-manual`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `filechanged-cwd`, `filechanged-nested`, `model-switch`, `model-switch-exit2`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `unauth`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `file_path`, `hook_event_name`, `load_reason`, `memory_type`, `session_id`, `transcript_path`.
...
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG InstructionsLoaded appears 0 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*
- `additionalContext` → **not-honored** — the model repeated 'PROBE_CTX InstructionsLoaded': no; the marker is in the transcript 0
0 time(s) (event fired in the run: 1 times) *(run additional-context)*

### `ConfigChange` — Claude Code

Fired 2 time(s) in run(s) `config-change`, `config-change-exit2`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `file_path`, `hook_event_name`, `prompt_id`, `session_id`, `source`, `transcript_path`.
...
- `exit2` → **unobservable** — hook_response: ; the file on disk keeps the edit (the block stops the reload, not the write): probeKey present = true *(run config-change-exit2)*

### `CwdChanged` — Claude Code

Fired 2 time(s) in run(s) `filechanged-cwd`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `new_cwd`, `old_cwd`, `prompt_id`, `session_id`, `transcript_path`.
...
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG CwdChanged appears 0 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*

### `DirectoryAdded` — Claude Code

**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run add-dir: the /add-dir command in -p (answer: /add-dir isn't available in this environment.); run add-dir-control: an SDK control_request register_repo_root over stream-json stdin (response: "error")

### `FileChanged` — Claude Code

Fired 6 time(s) in run(s) `filechanged-nested`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `event`, `file_path`, `hook_event_name`, `prompt_id`, `session_id`, `transcript_path`.
...
Fired 2 time(s) in run(s) `worktree`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `name`, `prompt_id`, `session_id`, `transcript_path`.
...
- `worktree` → **honored** — the session ran inside the path the hook echoed: yes; worktrees after the run: 2 *(run worktree)*

### `WorktreeRemove` — Claude Code

**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run worktree: the --worktree session exiting with a clean worktree; run worktree-agent: an Agent tool call with isolation worktree finishing

### `PreCompact` — Claude Code

Fired 2 time(s) in run(s) `compact-auto`, `compact-manual`. `hook_event_name`: present; `session_id`: present. Payload keys: `custom_instructions`, `cwd`, `hook_event_name`, `prompt_id`, `session_id`, `transcript_path`, `trigger`.
...
Fired 2 time(s) in run(s) `compact-auto`, `compact-manual`. `hook_event_name`: present; `session_id`: present. Payload keys: `compact_summary`, `cwd`, `hook_event_name`, `prompt_id`, `session_id`, `transcript_path`, `trigger`.
...
Fired 2 time(s) in run(s) `model-switch`, `model-switch-exit2`. `hook_event_name`: present; `session_id`: present. Payload keys: `cache_ttl`, `context_tokens`, `cwd`, `estimated_cache_write_usd`, `from_model`, `hook_event_name`, `pricing`, `prompt_cache_warm`, `prompt_id`, `requested_model`, `session_id`, `source`, `to_model`, `transcript_path`.
...
- `exit2` → **honored** — PostModelSwitch fired 0 times after the block (0 = the switch did not happen); final: Model switch to Sonnet 5 was blocked by a PreModelSwitch hook: [bash /Users/jonhvmp/myspace/github/leopold/scripts/probe/dump-hook.sh PreModelSwitch --out /priv *(run model-switch-exit2)*

### `PostModelSwitch` — Claude Code

Fired 1 time(s) in run(s) `model-switch`. `hook_event_name`: present; `session_id`: present. Payload keys: `cache_ttl`, `context_tokens`, `cwd`, `estimated_cache_write_usd`, `from_model`, `hook_event_name`, `pricing`, `prompt_cache_warm`, `prompt_id`, `requested_model`, `session_id`, `source`, `to_model`, `transcript_path`.
...
**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run elicit-observe: a stdio MCP stub whose tool sends elicitation/create back to the client

Replies the harness honored:

- `action:accept+content` → **not-honored** — the MCP server received the hook's answer: ; ElicitationResult fired: 0 (it fires only on the user/cancel path) *(run elicit-accept)*

### `ElicitationResult` — Claude Code

**Not fired on Claude Code 2.1.259 (Claude Code).** Trigger tried: run elicit-observe: the same elicitation, unanswered by any hook, in a headless session (auto-cancelled)
...
Fired 34 time(s) in run(s) `add-dir`, `additional-context`, `command`, `command-exit2`, `compact-auto`, `compact-manual`, `compact-manual-seed`, `config-change`, `config-change-exit2`, `fail-429`, `filechanged-cwd`, `filechanged-nested`, `model-switch`, `model-switch-exit2`, `perm-allow`, `perm-auto`, `perm-deny`, `perm-host`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `setup-init`, `setup-maintenance`, `stop-block`, `stop-exit2`, `subagent`, `subagent-exit2`, `system-message`, `tasks`, `tasks-exit2`, `teams`, `tools`, `worktree-agent`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `prompt_id`, `reason`, `session_id`, `transcript_path`.
...
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG SessionEnd appears 0 times in the stream-json output (outside hook_response echoes:  5 informational;); in the transcript:  2 PROBE_SYSMSG PostToolBatch; 2 PROBE_SYSMSG PostToolUse; 2 PROBE_SYSMSG PreToolUse; 2 PROBE_SYSMSG SessionStart; 2 PROBE_SYSMSG Stop; 1 PROBE_SYSMSG UserPromptSubmit; *(run system-message)*

## Events — Codex CLI

### `SessionStart` — Codex CLI

Fired 25 time(s) in run(s) `additional-context`, `compact`, `exec-as-role-4`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `interrupt`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `role-bogus`, `role-bogus-strict`, `stop-block`, `stop-exit2`, `strict-config`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `source`, `transcript_path`.
...
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG SessionStart in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*
- `additionalContext` → **not-honored** — the model repeated 'PROBE_CTX SessionStart': no *(run additional-context)*
...
Fired 24 time(s) in run(s) `additional-context`, `compact`, `exec-as-role-4`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `interrupt`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `role-bogus`, `role-bogus-strict`, `stop-block`, `stop-exit2`, `strict-config`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `reason`, `session_id`, `transcript_path`.
...
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG SessionEnd in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*

### `SubagentStart` — Codex CLI

Fired 3 time(s) in run(s) `subagent`, `subagent-exit2`, `subagent-role`. `hook_event_name`: present; `session_id`: present. Payload keys: `agent_id`, `agent_type`, `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `transcript_path`, `turn_id`.
...
- `role file` → **honored** — agent_type in SubagentStart: probe-reviewer; the role's fixed reply came back: yes *(run subagent-role)*
...
Fired 4 time(s) in run(s) `subagent`, `subagent-exit2`, `subagent-role`. `hook_event_name`: present; `session_id`: present. Payload keys: `agent_id`, `agent_transcript_path`, `agent_type`, `cwd`, `hook_event_name`, `last_assistant_message`, `model`, `permission_mode`, `session_id`, `stop_hook_active`, `transcript_path`, `turn_id`.
...
- `exit2` → **honored** — SubagentStop fired 2 times (exit 2 once); stop_hook_active on the second: true *(run subagent-exit2)*

### `PreToolUse` — Codex CLI

Fired 29 time(s) in run(s) `additional-context`, `compact`, `interrupt`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `pretool-deny`, `role-bogus`, `role-bogus-strict`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `tool_input`, `tool_name`, `tool_use_id`, `transcript_path`, `turn_id`.
...
- `permissionDecision:deny` → **honored** — PostToolUse fired 0 times after the deny (0 = the tool never ran); reason reached the model: yes *(run pretool-deny)*
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG PreToolUse in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*
- `additionalContext` → **not-honored** — the model repeated 'PROBE_CTX PreToolUse': no *(run additional-context)*

### `PermissionRequest` — Codex CLI

Fired 1 time(s) in run(s) `perm-deny`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `tool_input`, `tool_name`, `transcript_path`, `turn_id`.
...
- `decision:allow` → **not-honored** — the escalated curl ran (an HTTP status line came back): no; PermissionRequest fired: 0 *(run perm-allow)*
- `decision:deny` → **honored** — the deny message reached the model: yes; an HTTP status line came back: no; stderr: Rejected(\"PROBE_DENY\") *(run perm-deny)*
- `decision:allow (approval_policy=on-request)` → **not-honored** — under approval_policy=on-request in headless exec the hook fired 0 times; reply:  *(run perm-onrequest)*

### `PostToolUse` — Codex CLI

Fired 24 time(s) in run(s) `additional-context`, `compact`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `tool_input`, `tool_name`, `tool_response`, `tool_use_id`, `transcript_path`, `turn_id`.
...
- `decision:block` → **honored** — the reason PROBE_BLOCK reached the model/stream: yes; reply: ``` *(run posttool-block)*
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG PostToolUse in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*
- `additionalContext` → **honored** — the model repeated 'PROBE_CTX PostToolUse': yes *(run additional-context)*

### `PreCompact` — Codex CLI

Fired 1 time(s) in run(s) `compact`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `session_id`, `transcript_path`, `trigger`, `turn_id`.
...
Fired 24 time(s) in run(s) `additional-context`, `compact`, `exec-as-role-4`, `fail-401`, `fail-429`, `fail-500`, `fail-529`, `interrupt`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `pretool-deny`, `prompt-exit2`, `role-bogus`, `role-bogus-strict`, `stop-block`, `stop-exit2`, `strict-config`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `prompt`, `session_id`, `transcript_path`, `turn_id`.
...
- `exit2` → **honored** — the model answered the prompt: no; Stop fired: 0; errors:  *(run prompt-exit2)*
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG UserPromptSubmit in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*
- `additionalContext` → **not-honored** — the model repeated 'PROBE_CTX UserPromptSubmit': no *(run additional-context)*

### `Stop` — Codex CLI

Fired 20 time(s) in run(s) `additional-context`, `compact`, `exec-as-role-4`, `perm-allow`, `perm-deny`, `perm-onrequest`, `posttool-block`, `pretool-deny`, `role-bogus`, `role-bogus-strict`, `stop-block`, `stop-exit2`, `strict-config`, `subagent`, `subagent-exit2`, `subagent-role`, `system-message`, `tools`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `last_assistant_message`, `model`, `permission_mode`, `session_id`, `stop_hook_active`, `transcript_path`, `turn_id`.
...
- `exit2` → **honored** — Stop fired 2 times (exit 2 once); stop_hook_active on the second: true; messages: 2 *(run stop-exit2)*
- `decision:block` → **honored** — Stop fired 2 times ({"decision":"block"} once); stop_hook_active on the second: true *(run stop-block)*
- `systemMessage` → **unobservable** — marker PROBE_SYSMSG Stop in the --json stream: 0 times; in the rollout transcript: 0
0 times *(run system-message)*
...
Fired 1 time(s) in run(s) `interrupt`. `hook_event_name`: present; `session_id`: present. Payload keys: `cwd`, `hook_event_name`, `model`, `permission_mode`, `session_id`, `transcript_path`, `turn_id`.
...
Every write landed under the `mktemp -d` root (`/tmp/leopold-hookprobe.gjQnXe`), under `<out>`, or in the harnesses' own transcript directories for that temp project. The real `~/.claude` and `~/.codex` entry-name fingerprints were identical before and after (unchanged / CHANGED); the temp `CODEX_HOME`'s copied `auth.json` was deleted when the probe ended. Rerun: `bash scripts/probe-hook-events.sh --out <dir>` (never part of `make test`; linted by `make hooks-check`).
