...
- [ ] (after: 1) Ship it
- [ ] @human Approve the rollback  # a role Leopold synthesizes decides it
```

`@gate` / `@verify` review without touching a file, `@tool` runs a command with no model turn, `@human` is decided by a role Leopold synthesizes from the item and your charter — and recorded in `DECISIONS.md` with a Reversal — unless you set `autonomy: ask`, `@feedback` may amend the plan within bounds it cannot exceed. **No model call decides an edge** — a node emits a signal, the graph decides where it leads, which is what makes the Canvas worth trusting. `leopold graph` prints and validates before a single agent runs: a cycle or a route to an item that does not exist exits 1 naming the offender, so a malformed plan costs zero tokens. See [Plan Grammar](docs/reference/plan-grammar.md).

**Nothing halts — Leopold assumes the role and does the work.** Where a run used to hand work back to a person, it now works out **who** that decision needs and becomes them. A `@human` node, an escalation, a plan graph that will not validate, a third failure of the same kind: Leopold **synthesizes the role** from the item plus your `CHARTER.md` — a name, the expertise the item actually demands, the charter rules lifted verbatim to bind it — takes it, does the work, and records the call in `DECISIONS.md` with a **Reversal** line. Every run ends with *"What I decided for you"*, riskiest first. **The persona decides; it never ships** — the git lock does not move, and no persona may raise a budget, clear the kill switch or edit `GUARDRAILS.md`. Budgets, the kill switch and an unsettleable fork still stop the run; nothing else does. Prefer the old behaviour? `autonomy: ask` in `GUARDRAILS.md` restores the halting `@human` on both engines. See [Personas](docs/concepts/personas.md).
...
- **Graph pre-flight.** `leopold-driver graph` prints the plan as the graph the scheduler actually routes on — node kinds, `(after:)` edges, conditional `@on` routes, signals — and validates it, *before the first agent runs*. A cycle, a route to an item that does not exist, an unreachable item or an unmet `@needs` exits non-zero with the offending items named. An `@on` route naming a signal nothing `@emit`s is reported as a warning — the edge can never fire, so you hear about it every time, but a plan that ran yesterday still runs. `--mermaid` for a diagram, `--quiet` for a script gate.
...
- **The run survives context pressure.** Since 0.18.0 a full context window is **maintenance, not death**: the run writes a structured `.leopold/CHECKPOINT.md`, the window rolls, and the next window continues the plan — relaunched automatically by `leopold watch` under `continuity: auto` (the default), no human in the loop. The governor is **durable progress**, not a cost counter: two consecutive windows closing zero plan items stop the run honestly (`no_progress_across_windows`), `max_iterations` is the run's ceiling across windows, and `max_windows` (default 10) bounds total reseeds. The kill switch beats auto-relaunch, always. See [Continuity](docs/concepts/continuity.md).
- **Cost-bounded, honestly.** USD is deliberately **not** the governor — `total_cost_usd` lies on subscription billing — so the run is bounded by progress gates and iteration/window ceilings instead. API-billed users who want a hard dollar cap can opt in with the driver's **`--budget <usd>` hard-stop**. The protocol keeps the orchestrator lean — bulk-output work is delegated to subagents that **write to files**, so output never piles up in the main context.
- **Fails closed.** A malformed run-state file blocks loudly; it never silently lets autonomy through.
- **Kill switch + audit.** `/leopold-stop` (or `touch .leopold/STOP`) halts at the next turn boundary; every autonomous decision is logged with its reasoning — including the ones a synthesized [persona](docs/concepts/personas.md) made, which name the role, the fork and how you undo it.
